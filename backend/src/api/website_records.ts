...
class WebsiteRecordsAPI {
  ...
  constructor(app: Express, db: Pool) {
    this.app = app;
    this.db = db;
  }
  ...
  public init() {
    ...
    this.app.get("/website-records", async (req: Request, res: Response) => {
      const query = "SELECT * FROM website_records;";

      this.db.query(query, function (error, results, _) {
        if (error) {
    ...
    this.app.delete("/delete-website-record/:id", async (req, res) => {
    ...
    this.app.post(
      ...
      async (req: Request, res: Response) => {
        ...
        this.db.query(query, params, function (error, results: OkPacket, _) {
          if (error) {
            res.status(500).send({ errorMsg: error });
            return;
          }
    ...
    this.app.post(
      ...
      async (req: Request, res: Response) => {
        const record: WebsiteRecord = this.validateAndParseWebsiteRecord(req.body);
  ...
  private validateAndParseWebsiteRecord(websiteRecord): WebsiteRecord {
    if (
  ...
  const validation = {
      'url': () => typeof field === "string" && (field.startsWith('https://') || field.startsWith('http://')),
