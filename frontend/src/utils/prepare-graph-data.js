export const getPreparedDataForGraphVisualisation = ({
  graphData,
  isRequestedWebsiteView,
}) => {
      ...
      if (!sameNode.recordIdList.find((id) => id === inputNode.recordId)) {
        newNodes = newNodes.map((node) => {
          if (node.url === inputNode.url) {
            return {
              ...node,
              recordIdList: [...node.recordIdList, inputNode.recordId],
            };
          }
          return node;
        });
...
const getPreparedGraphDomainView = ({ graphData }) => {
  ...
  const chechAndAddNewDomainNodeToNodeList = (inputNode, inputDomain) => {
    ...
    if (!sameDomainNode) {
      ...
      if (
        !sameDomainNode.listRecordsCrawledThisDomain.find(
          (id) => id === inputNode.recordId
        )
      ) {
        newNodes = newNodes.map((node) => {
